...
public class ResponseController {
	...
	public void response(Model model) {
		...
		model.addAttribute("board", "World");
	}
	...
	/**
	 * 요청 경로  : http://localhost:8080/response/model/view
	 * 응답 	   : /response/index.jsp 
	 * @return
	 */
	...
	/**
	 * 요청 경로  : http://localhost:8080/response/data/board
	 * 응답 	   : board (JSON/XML)
	 * @ResponseBody : 응답하는 데이터를 응답 메시지의 body(본문) 에 지정하는 어노테이션
	 * @return
	 */
	@ResponseBody
	@RequestMapping(value = "/data/board") // default xml
	//@RequestMapping(value = "/data/board", produces = "application/json")
	public Board responseBoard() {
		Board board = new Board(0, "제목","작성자","내용");
		return board;
	}
	 ...
	 * @throws Exception
	 */
	@ResponseBody   // 없는 경우 /data/board/list.html 파일을 찾음.
	//@RequestMapping("/data/board/list") // default xml
	...
	public Map<String, Board> responseMap() throws Exception {
		...
		map.put("board1", new Board(1, "제목1","작성자1","내용1"));
		map.put("board2", new Board(2, "제목2","작성자2","내용2"));
		map.put("board3", new Board(3, "제목3","작성자3","내용3"));
		
		return map;
	}
	
	// http://localhost:8080/response/data/entity/void
	@ResponseBody
	...
	public ResponseEntity<byte[]> responseFile(HttpServletRequest request) throws Exception {
		...
		String path = request.getServletContext().getRealPath("/templates/upload/logo.png");
		logger.info("path : " + path);
		
		// 파일 경로
		String filePath = "D:\\2ndLife\\dwitter\\public\\img\\logo.png";
		//String filePath = request.getServletContext().getRealPath("/upload/logo.png");
