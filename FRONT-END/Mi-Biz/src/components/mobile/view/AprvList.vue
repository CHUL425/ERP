...
async function searchData() {
    const res: any = await api.mobile.selectIng(dataParam.value);
    ...
    if (res?.data?.length) {
        ...
        if (route.query.rctNo) {
            const check = dataUI.value.listData.some((element: any) => {
                ...
                dataUI.value.selectItem.aprvRnk = element.aprvRnk;
                dataUI.value.selectItem.aprvOrzCd = element.aprvOrzCd;
            });

            if (check === false) {
                if (token.value) {
                    router.replace({ name: 'rot', query: { CAUTK: userCd.value, mnTyp: '', rctNo: '' } });
                }
                if (userId.value) {
                    router.replace({ name: 'rot', query: { ID: userCd.value, mnTyp: '', rctNo: '' } });
                }
...
function toggleAccordion(item: any) {
    dataUI.value.listData.some((element: any) => {
        ...
        dataUI.value.selectItem.aprvRnk = item.aprvRnk;
        dataUI.value.selectItem.aprvOrzCd = item.aprvOrzCd;
    });
}
</script>
